...
disp('Simulating Lcontainer.m  with UUV-like parameters under PD-control with psi_ref=[-15 to 15] (deg) ...')
...
for j = 1:sims
    ...
    for i=1:N+1
        ...
        [xdot,U] = LContainer_uuv(x,delta_c);  % ship model, see .../gnc/VesselModels/
        xderi(i,:) = xdot; 
...
ylabel('p (rad/s))')
title('Roll velocity');
saveas(fi,'figures/rollvsim','png');
