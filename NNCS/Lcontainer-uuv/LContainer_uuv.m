function [xdot,U] = LContainer_uuv(x,ui)
...
delta_c = max(min(delta_max*pi/180,delta_c),-delta_max*pi/180);
delta_dot = delta_c - delta;
delta_dot = max(min(Delta_max*pi/180,delta_dot),-Delta_max*pi/180);

% TM'inv(T) dv/dt + (U/L) TN'inv(T) v + (U/L)^2 TG'inv(T) eta = (U^2/L) T b' delta
% nudot = inv(T*M*Tinv)*((U^2/L)*T*b*delta-(U/L)*T*N*Tinv*nu-(U/L)^2*T*G*Tinv*eta);
nudot = eye(3)/(T*M*Tinv)*((U^2/L)*T*b*delta-(U/L)*T*N*Tinv*nu-(U/L)^2*T*G*Tinv*eta);
